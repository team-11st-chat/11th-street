...
## 6. 이슈 #29 종료 조건과 본 PR의 범위

이슈 #29 세부 작업에는 "1단계 실패 테스트(`couponOverIssuingReproduced`)가 Lock 적용 후 통과(GREEN)하는지 검증"이 포함된다.
이 GREEN 검증은 실제 Redis/Redisson 인프라가 필요하므로 본 PR에서는 수행하지 못했다.

따라서 본 PR은 락 구현의 **추적·문서화**까지만 책임지며, **이슈 #29를 종료하지 않는다**(`Refs #29`).
GREEN 검증은 통합 테스트 인프라가 갖춰진 뒤 별도로 수행하고, 그 결과로 #29를 종료한다.
