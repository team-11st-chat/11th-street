...
| 락 잔류(보유 중 프로세스 사망) | 분산 락은 해제 누락 시 영구 점유 | `@Value`로 외부화된 Lease Time 설정(기본 5초) 적용 | lock-strategy §6 |
...
- **임계구역 길이 vs Lease**: 임계구역이 Lease(5초)보다 길어지면 락이 먼저 풀려 보호가 깨진다. → 임계구역을 수량 차감·생성으로 최소화하는 원칙을 유지한다.
...
| Lock 임계 만료(Lease) | 5초 | **외부화 채택**. `app.lock.default-lease-seconds` 설정(기본 5초)과 일치 |
